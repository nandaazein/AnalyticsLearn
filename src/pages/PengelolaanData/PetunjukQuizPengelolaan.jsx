...
export default function PetunjukPengerjaanPengelolaan() {
  const [history, setHistory] = useState([]);
  const [error, setError] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  const navigate = useNavigate();

  // Fetch quiz attempt history for Kuis 4 only
  useEffect(() => {
    const fetchQuizHistory = async () => {
      try {
        ...
        // Filter for Kuis 4 attempts and construct history array
        const historyData = attempts
          .filter((attempt) => attempt.quizNumber === 4)
          .map((attempt) => ({
      ...
      <div className="flex items-start justify-center flex-1 p-6 bg-gray-100">
        <div className="w-full max-w-4xl p-8 bg-white rounded-lg shadow-lg">
          ...
          <div className="flex justify-center gap-4 mt-6">
            <button
              onClick={() => (window.location.href = "/kuis-pengelolaan")}
          ...
          {/* History Table for Kuis 4 */}
          <div className="max-w-4xl mx-auto mt-8 bg-white rounded-md shadow-md">
            <div className="px-4 py-2 border-b border-gray-300">
              <h2 className="font-semibold text-gray-900">Riwayat Kuis 4</h2>
            </div>
            ...
            {isLoading ? (
              ...
              <p className="px-4 py-2 text-sm text-gray-600">
                Belum ada riwayat pengerjaan Kuis 4.
              </p>
            ) : (
